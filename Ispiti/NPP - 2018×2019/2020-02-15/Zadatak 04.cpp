...
void vremenska_prognoza::Unos()
{
    ...
    int velicinaP = strlen(tempPadavine);
    padavine = new char[velicinaP];
    strcpy(padavine, tempPadavine);
...
int najtacniji_meteorolog
(
    vremenska_prognoza **matrica, 
    float *stvarne_izmjerene_vrijednosti,
    int brMeteorologa,
    int brDana
)
{
    float *odstupanja = new float[brMeteorologa];
    for(int i = 0; i < brMeteorologa; i++)
        for(int j = 0; j < brDana; j++)
        {
            *(odstupanja + i) += abs
            (
                *(*(matrica + i) + j)->temperatura - *(stvarne_izmjerene_vrijednosti + i)
            );
        }
